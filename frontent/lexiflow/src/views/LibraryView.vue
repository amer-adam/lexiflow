...
const fetchLibrary = async () => {
  try {
    ...
    const response = await fetch(`https://api.amerai.top/lexiflow/library?user_id=${userId}`);
...
const executeSearch = async () => {
  ...
  if (searchMode.value !== 'titles') {
    try {
      const res = await fetch(`https://api.amerai.top/lexiflow/search?word=${encodeURIComponent(searchQuery.value)}`);
      if (res.ok) {
